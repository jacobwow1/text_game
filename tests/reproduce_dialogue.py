...
def test_dialogue_logic():
    ...
    # Initially, no flags. Should get "chapter_1/chen_default.json"
    game.update_hub_options()
    options = game.state["hub_options"]
    chen_option = next((o for o in options if "Advisor Chen" in o["text"]), None)
    
    if chen_option and chen_option["target"] == "chapter_1/chen_default.json":
        print("PASS: Correctly selected default dialogue.")
    else:
        print(f"FAIL: Expected chapter_1/chen_default.json, got {chen_option['target'] if chen_option else 'None'}")

    print("\n--- Test Case 2: Dialogue Completion & Fallback ---")
    # Mark default as completed
    player.add_completed_dialogue("chapter_1/chen_default.json")
    ...
    if chen_option and chen_option["target"] == "common/chen_generic.json":
        print("PASS: Correctly fell back to generic dialogue.")
    else:
        print(f"FAIL: Expected common/chen_generic.json, got {chen_option['target'] if chen_option else 'None'}")
    ...
    if chen_option and chen_option["target"] == "chapter_1/chen_secret.json":
        print("PASS: Correctly selected secret dialogue after flag set.")
    else:
        print(f"FAIL: Expected chapter_1/chen_secret.json, got {chen_option['target'] if chen_option else 'None'}")

    print("\n--- Test Case 4: All Completed ---")
    # Mark secret as completed too
    player.add_completed_dialogue("chapter_1/chen_secret.json")
